...
export default function MenuCategory({ slug, label, description, items }: MenuCategoryProps) {
  ...
  return (
    <section id={`category-${slug}`} className="py-12 lg:py-16">
      <h2 className="font-['Anton',sans-serif] text-3xl lg:text-[40px] text-[#1e170e] mb-2 uppercase">
        {label}
      </h2>
      {description && (
        <p className="font-['Inter',sans-serif] text-[#484037] text-base mb-8">{description}</p>
      )}

      {Object.entries(grouped).map(([subcat, subcatItems]) => (
        <div key={subcat} className="mb-8">
          {hasSubcategories && subcat !== '__default' && (
            <h3 className="font-['Inter',sans-serif] font-medium text-sm uppercase tracking-widest text-[#484037] mb-4 pb-2 border-b border-[#1e170e]/10">
              {subcat}
            </h3>
          )}
          <div>
